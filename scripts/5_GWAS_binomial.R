...
# Retrieve the results of the implicit feature selection performed by LASSO by identifying
# the variables that have non-zero coefficients.
res <- which(beta[-1] !=0)
length(res) # no significant SNPs

# Plot the fraction of deviance
...
# Inspect The corrected SNPs matrix:
dim(snps.corrected)
range(snps.corrected)

# To visually assess whether the correction for population stratification has been successful,
# run a second PCA analysis, this time with the corrected SNPs matrix:
...
# two stages. First, generate a simple linear model between each column of the corrected
# SNPs matrix and the phenotypic trait. Second, run ANOVA on
...
# The LASSO method generates coefficients for each variable, though the majority of these
# will have been shrunk to zero in the penalization step. Extract these coefficients from the
# model generated by cv.glmnet with the function coef.
